...
def cleanup_test_users(db_conn):
    """Yield, then wipe test users + everything they own.

    Order matters because of FK constraints. exam_attempts has no
    cascade from exam, so it must be deleted first. exam_questions
    cascade from exam. courses cascade their chapters/episodes/materials
    but not exams.
    """
    yield
    cur = db_conn.cursor()
    prefix = (f"{settings.TEST_EMAIL_PREFIX}%",)
    # Attempts owned by test students
    cur.execute(
        "DELETE ea FROM exam_attempts ea JOIN users u ON ea.user_id = u.id "
        "WHERE u.email LIKE %s",
        prefix,
    )
    # Attempts on exams whose teacher is a test user
    cur.execute(
        "DELETE ea FROM exam_attempts ea JOIN exams e ON ea.exam_id = e.id "
        "JOIN users u ON e.teacher_id = u.id WHERE u.email LIKE %s",
        prefix,
    )
    # Exams owned by test teachers (questions cascade)
    cur.execute(
        "DELETE e FROM exams e JOIN users u ON e.teacher_id = u.id "
        "WHERE u.email LIKE %s",
        prefix,
    )
    # Courses owned by test teachers (chapters/episodes/materials cascade)
    cur.execute(
        "DELETE c FROM courses c JOIN users u ON c.teacher_id = u.id "
        "WHERE u.email LIKE %s",
        prefix,
    )
    cur.execute("DELETE FROM users WHERE email LIKE %s", prefix)
    db_conn.commit()
    cur.close()
